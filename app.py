        ...
        for selected_id in all_ids:
            for page_num, page in enumerate(pdf_przepisy.pages):
                ...
                if info:
                    pdf.add_page()
                    pdf.set_font("DejaVu", "", 16)
                    pdf.cell(0, 10, f"Przepis ID {selected_id} - {info['tytul']}", ln=True)
                    pdf.ln(5)
                    pdf.set_font("DejaVu", "", 12)
                    pdf.multi_cell(0, 8, f"DIETA: {info['dieta']}\nWARIANT: {info['wariant']}\nPOSIŁEK: {info['posilek']}\nDANIA: {info['dania']}\nZAMÓWIONO: {info['zamowiono']}\nZAMÓWIONO (WpOT): {info['zamowiono_wpot']}\n\nOpis przygotowania:\n{info['opis']}\n")
                    # Składniki (jeśli są)
                    if info['skladniki'] is not None and not info['skladniki'].empty:
                        pdf.multi_cell(0, 8, "\nSKŁADNIKI DLA PRZEPISU:\n" + info['skladniki'].to_string(index=False))
                    # Obraz (jeśli jest)
...
def export_all_gramatury_pdf():
    ...
    with pdfplumber.open(tmp_gramatury_path) as pdf_gramatury:
        for selected_id in all_ids:
            ...
            pdf.add_page()
            pdf.set_font("DejaVu", "", 16)
            pdf.cell(0, 10, f"Gramatura ID {selected_id} - {nazwa_dania}", ln=True)
            pdf.ln(5)
            pdf.set_font("DejaVu", "", 12)
            pdf.multi_cell(0, 8, f"{gramatura}\n\n")
            # Obraz (jeśli jest)
            ...
            if gramatura_img is not None:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp:
                    img_path = tmp.name
                    gramatura_img.save(img_path, format='PNG')
                pdf.add_page()
                pdf.image(img_path, x=10, y=30, w=180)
                os.unlink(img_path)
    return pdf.output(dest="S").encode("latin1")
...
excel_data = export_df_to_excel(df_sorted, "Lista_składników")
st.download_button(
    ...
    with pdfplumber.open(tmp_przepisy_path) as pdf_przepisy:
        ...
        if przepisy:
            for idx, przepis in enumerate(przepisy, 1):
                ...
                # --- PODŚWIETLENIE NAJWIĘKSZEJ MASY/WAGI ---
...
for selected_id in all_ids:
    with pdfplumber.open(tmp_gramatury_path) as pdf_gramatury:
        ...
        st.markdown(
            f'<h3 style="color:#43A047; margin-bottom: 0.5em;">Gramatura dla ID {selected_id} – {nazwa_dania}</h3>',
            unsafe_allow_html=True
        )
        st.text(gramatura)
        ...
        if gramatura_img is not None:
            ...
            if ingredients:
                results = calculate_pre_cooking_weights(ingredients, df_skladniki)
